...
function getFlipMultiplier(layer) {
    ...
    // Считаем масштаб по X
    var sx = Math.sqrt(a * a + c * c);
    
    // Считаем детерминант (определяет, есть ли зеркальное отражение)
    var det = a * d - b * c;

    // --- ИЗМЕНЕНИЕ ---
    // Если есть отражение (det < 0), мы принудительно делаем отрицательным Scale X (Flip Horizontal),
    // а не Scale Y. Это более естественно для дизайнеров и лучше работает с углами.
    if (det < 0) {
        sx = -sx;
    }

    // Scale Y теперь будет положительным (так как neg / neg = pos)
...
function getRotation(layer) {
    ...
    // Считаем детерминант снова, чтобы узнать про флип
    var det = a * d - b * c;
    
    // Считаем угол вектора X
    var angle = Math.atan2(c, a);
    
    // --- ИЗМЕНЕНИЕ ---
    // Если мы инвертировали Scale X (в функции выше), то наш вектор (a, c)
    // на самом деле смотрит в противоположную сторону от "визуального" угла.
    // Нам нужно повернуть его на 180 градусов (PI), чтобы вернуть на место.
    if (det < 0) {
        angle += Math.PI;
    }

...
function getFlipMultiplier(layer) {
    ...
    // Считаем честную длину векторов (Magnitude)
    var sx = Math.sqrt(a * a + c * c);
    var sy = Math.sqrt(b * b + d * d); // Исправлено: теперь считаем Y независимо от детерминанта (чинит размер головы)
    
    // Определяем, есть ли отражение
    var det = a * d - b * c;

    // Если есть отражение, инвертируем Scale X
    if (det < 0) {
        sx = -sx;
    }
...
function getRotation(layer) {
    ...
    // Если Scale X отрицательный (det < 0), то вектор (a, c) физически смотрит 
    // в противоположную сторону от того, как если бы Scale был положительным.
    // Однако, atan2(c, a) возвращает угол именно этого вектора.
    
    // В AE порядок трансформаций: Scale -> Rotation.
    // Если мы делаем Scale [-100, 100], мы зеркалим картинку.
    // Угол, полученный через atan2(c, a), для зеркальной матрицы обычно корректен 
    // и не требует сдвига на 180, если мы правильно посчитали Scale.
    
    var angle = Math.atan2(c, a);
    
    // Но есть нюанс: если det < 0 (отражение), математика atan2 может дать угол, 
    // который для AE (с его Y-down системой) нужно инвертировать или сдвигать.
    // Самый надежный способ для Figma -> AE при Scale X < 0:
    
    var det = a * d - b * c;
    if (det < 0) {
        // Если мы отразили по X, нам нужно отзеркалить и вектор угла?
        // Нет, эмпирически для AE работает именно "сырой" угол вектора, 
        // но так как sx отрицательный, вектор "смотрит назад".
        // Чтобы компенсировать "взгляд назад" отрицательного масштаба, добавляем PI.
        angle += Math.PI;
    }
